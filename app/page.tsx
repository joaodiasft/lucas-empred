...
import { AccessAccount, AppSettings, Client, GeoLocation, Installment, Loan, Page, PayFrequency, Reference, Role, TeamMember, callHref, compressImage, currency, daysLate, defaultSettings, digitsOnly, formatAddress, frequencyLabel, generateInstallments, initials, isClientActive, loanBalance, loanFrequency, mapEmbedUrl, mapOpenUrl, payableAmount, periodLabel, persistGet, persistSet, referenceHref, riskFor, seedAccounts, seedClients, seedLoans, seedTeam, shortDate, uid, whatsappHref } from './lib';
...
type ModalState = { type:'contract'|'renegotiate'|'payment'|'team'|'confirm-payment'|'reject-payment'|'inactivate-client'; loanId?:string; installmentId?:string; clientId?:string } | null;
...
export default function App() {
  ...
  switch (page) {
    ...
    case 'calendar': view = <CalendarView clients={clients} loans={loans} onOpenLoan={id => navigate('loan-detail', id)} />; break;
    case 'collections': view = <CollectionsView clients={clients} loans={loans} settings={settings} onOpenLoan={id => navigate('loan-detail', id)} notify={notify} />; break;
...
function AdminHome({ clients, loans, role, onNavigate, onNewLoan }: { clients:Client[]; loans:Loan[]; role:Role; onNavigate:(page:Page,id?:string)=>void; onNewLoan:()=>void }) {
  ...
  return <>
    ...
    <section className="quick-actions"><button onClick={()=>onNavigate('new-client')}><span>＋</span><b>Adicionar cliente</b><small>Cadastre dados e referências</small></button><button onClick={onNewLoan}><span>↗</span><b>Novo empréstimo</b><small>Semanal ou mensal</small></button><button onClick={()=>onNavigate('collections')}><span>◉</span><b>Central de cobranças</b><small>Fila e mensagens prontas</small></button><button onClick={()=>onNavigate('reports')}><span>▦</span><b>Relatórios</b><small>Filtre e exporte sua carteira</small></button></section>
  </>;
}
...
function SettingsView({value,onSave}:{value:AppSettings;onSave:(value:AppSettings)=>void}) {
  ...
  return <><PageHeader eyebrow="PREFERÊNCIAS" title="Configurações" subtitle="Dados da empresa, recebimento e regras padrão."/><form className="settings-layout" onSubmit={event=>{event.preventDefault();onSave(form)}}><section className="form-card"><div className="form-section-title"><span>1</span><div><h3>Dados da empresa</h3><p>Informações exibidas nos contratos e no acesso do cliente.</p></div></div><div className="form-grid"><label>Nome da empresa<input value={form.companyName} onChange={e=>update('companyName',e.target.value)}/></label><label>CPF/CNPJ<input value={form.document} onChange={e=>update('document',e.target.value)}/></label><label>Telefone<input value={form.phone} onChange={e=>update('phone',e.target.value)}/></label><label>Chave PIX<input value={form.pixKey} onChange={e=>update('pixKey',e.target.value)}/></label></div></section><section className="form-card"><div className="form-section-title"><span>2</span><div><h3>Regras financeiras padrão</h3><p>Estes valores já vêm preenchidos ao criar um empréstimo. Você ainda pode mudar em cada contrato.</p></div></div><div className="form-grid"><FieldBlock title="Juros padrão (%)" hint="Taxa sugerida sobre o valor emprestado. Ex.: 20% em R$ 1.000 gera R$ 200 de juros. Vale só para novos contratos."><input type="number" step=".01" value={form.defaultRate} onChange={e=>update('defaultRate',Number(e.target.value))}/></FieldBlock><FieldBlock title="Prazo padrão" hint="Quantidade de parcelas sugerida. Se a periodicidade for semanal, são semanas. Se for mensal, são meses."><input type="number" value={form.defaultWeeks} onChange={e=>update('defaultWeeks',Number(e.target.value))}/></FieldBlock><FieldBlock title="Periodicidade padrão" hint="Ritmo de cobrança dos novos empréstimos. Semanal = uma parcela a cada 7 dias. Mensal = uma parcela por mês."><select value={form.defaultFrequency || 'weekly'} onChange={e=>update('defaultFrequency',e.target.value as PayFrequency)}><option value="weekly">Semanal</option><option value="monthly">Mensal</option></select></FieldBlock><FieldBlock title="Tipo de multa" hint="Como cobrar quando a parcela atrasa. Percentual usa % da parcela. Fixa cobra sempre o mesmo valor em reais."><select value={form.feeType} onChange={e=>update('feeType',e.target.value as 'fixed'|'percent')}><option value="percent">Percentual sobre a parcela</option><option value="fixed">Valor fixo em reais</option></select></FieldBlock><FieldBlock title={form.feeType==='percent'?'Valor da multa (%)':'Valor da multa (R$)'} hint={form.feeType==='percent'?'Percentual cobrado uma vez em cada parcela atrasada, além dos juros de mora.':'Valor em reais cobrado uma vez em cada parcela atrasada, além dos juros de mora.'}><input type="number" step=".01" value={form.feeValue} onChange={e=>update('feeValue',Number(e.target.value))}/></FieldBlock><FieldBlock title="Mora ao dia (%)" hint="Percentual extra por cada dia de atraso, em cima do valor da parcela. Ex.: 0,033% ao dia rende cerca de 1% ao mês."><input type="number" step=".001" value={form.lateInterest} onChange={e=>update('lateInterest',Number(e.target.value))}/></FieldBlock><FieldBlock className="span-2" title="Lembrete antes do vencimento (dias)" hint="Quantos dias antes do vencimento o sistema avisa. Não altera juros, multa nem mora — só o aviso."><input type="number" value={form.reminderDays} onChange={e=>update('reminderDays',Number(e.target.value))}/></FieldBlock></div></section><div className="settings-save"><p>As alterações valem para novos contratos. Os contratos atuais mantêm suas próprias regras.</p><button className="primary-button">Salvar configurações</button></div></form></>;
}
...
function RenegotiateModal({loan,settings,onSave,onClose}:{loan:Loan;settings:AppSettings;onSave:(loan:Loan)=>void;onClose:()=>void}) {
  const balance=loanBalance(loan);const [rate,setRate]=useState(settings.defaultRate);const [weeks,setWeeks]=useState(settings.defaultWeeks);const [firstDue,setFirstDue]=useState(()=>{const d=new Date();d.setDate(d.getDate()+7);return d.toISOString().slice(0,10)});const preview=generateInstallments({principal:balance,rate,interestMode:'total',weeks,firstDueDate:firstDue,frequency:loanFrequency(loan)});
  return <Modal title="Renegociar empréstimo" onClose={onClose}><form onSubmit={event=>{event.preventDefault();const id=uid('ren');onSave({id,clientId:loan.clientId,contractNumber:`LE-R-${id.slice(-4).toUpperCase()}`,principal:balance,rate,interestMode:'total',weeks,firstDueDate:firstDue,frequency:loanFrequency(loan),feeType:loan.feeType,feeValue:loan.feeValue,lateInterest:loan.lateInterest,status:'Ativo',installments:preview,createdAt:new Date().toISOString().slice(0,10),originalLoanId:loan.id})}}><div className="renegotiation-balance"><small>Saldo consolidado com encargos</small><b>{currency(balance)}</b><span>Este valor vira o novo principal. Os juros abaixo são cobrados em cima dele, não do contrato antigo.</span></div><div className="form-grid"><FieldBlock title="Nova taxa (%)" hint="Percentual de juros da renegociação, cobrado sobre o saldo em aberto. Ex.: 20% em R$ 1.000 gera R$ 200 de juros no novo contrato."><input type="number" step=".01" value={rate} onChange={e=>setRate(Number(e.target.value))}/></FieldBlock><FieldBlock title={`Novo prazo (${loanFrequency(loan)==='monthly'?'meses':'semanas'})`} hint="Quantas parcelas o cliente terá para quitar o saldo. A periodicidade continua a mesma do contrato original."><input type="number" min="1" value={weeks} onChange={e=>setWeeks(Number(e.target.value))}/></FieldBlock><FieldBlock className="span-2" title="Primeiro vencimento" hint="Data da primeira parcela do novo acordo. As demais seguem no mesmo ritmo do contrato original."><input type="date" value={firstDue} onChange={e=>setFirstDue(e.target.value)}/></FieldBlock></div><div className="simulation-mini"><span><small>Nova parcela</small><b>{currency(preview[0]?.amount||0)}</b></span><span><small>Novo total</small><b>{currency(preview.reduce((s,i)=>s+i.amount,0))}</b></span></div><div className="modal-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button">Criar renegociação</button></div></form></Modal>;
}
